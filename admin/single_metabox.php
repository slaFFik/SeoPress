...
function sp_post_metabox(){
    global $post;
    $title_field = $description_field = $keywords_field = $noindex_field = $post_metabox_table = '';
    ...
    if( isset($post_meta['title']) && $post_meta['title'] != '' ){
        $preview_title = $post_meta['title'];
    }else{
        $preview_title = $post->post_title;
    }
    
    if( isset($post_meta['description']) && $post_meta['description'] != '' ){
